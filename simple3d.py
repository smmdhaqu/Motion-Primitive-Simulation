...
def update_lines(frame, lines, ldd, ax):
    for i, line in enumerate(lines):
        line.set_data_3d(ldd[i].loc[:frame, 'X'], ldd[i].loc[:frame, 'Y'], ldd[i].loc[:frame, 'Z'])
...
for i, ll in enumerate(ld):
    line, = ax.plot(ll.loc[0, 'X'], ll.loc[0, 'Y'], ll.loc[0, 'Z'], label=f'{files[i]}')
    lines.append(line)

ax.legend()
ani = animation.FuncAnimation(
    fig, update_lines, min_row_l.index[min_row_l.index.levels[0] % 8 == 0], fargs=(lines, ld, ax), interval=(1000/15), blit=True)

# plt.show()
ani.save('all_3d_same_start.mp4', fps=15, dpi=300)
