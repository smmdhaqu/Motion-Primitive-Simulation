...
def update_lines(frame, d ,ls, ax):
    ls.set_segments(d[:, :(frame[0]+1), [0, 2]])
    # tx.set_text(frame[1])
...
for fl in files:
    ...
    ax.set_ylim(lim_ref[fl][5], lim_ref[fl][4])
    ...
    line_segments = LineCollection(segs[:, :1, [0, 2]], linestyles='solid', colors=colors)
    ...
    ani.save(f'{fl}_XZ_2d.mp4', fps=15, dpi=300)
